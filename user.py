...
class User:
    def __init__(self, rh, ticker, shares_to_buy):
        ...
        # Not sure which var to pull from portfolios for buying power, test the following:
        # excess_margin, excess_maintenance, excess_margin_with_uncleared_deposits, excess_maintenance_with_uncleared_deposits
        self.buying_power = float(rh.portfolios()["excess_margin"])
        #self.buying_power = float(rh.portfolios()["withdrawable_amount"]) # Dont think this works doe to money needed 2 days to settle after a trade is exited.
        #self.buying_power = float(rh.portfolios()["last_core_equity"]) # This doesnt work either because its including stocks owed. 
        ...
        self.estimated_trans_cost = self.latest_trade_price * self.shares_to_buy
        self.gainsloses = round((self.latest_trade_price *  self.shares_owned) - self.average,2)
...
def get_last_sp(self, rh, ticker):
    # Get stock quote
    print("Getting latest stock price")
